...
These tests cover:
 ...
 - IRBuilder (insertion-point management, op insertion, block creation)
...
def _make_simple_module():
    ...
    module = pliron.ModuleOp.new("test_mod").operation()
    func = pliron.FuncOp.new("foo", func_ty).operation()
...
class TestTypes:
    ...
    def test_integer_type_invalid_signedness(self):
        with pliron.Context():
            with pytest.raises(pliron.PlironError, match="Invalid signedness"):
                pliron.IntegerType.get(32, "bogus")

    def test_function_type(self):
        with pliron.Context():
            i32 = pliron.IntegerType.get(32)
            i64 = pliron.IntegerType.get(64, "signed")
            fty = pliron.FunctionType.get([i32], [i64])
            s = str(fty)
            assert "i32" in s
            assert "si64" in s
    ...
    def test_unit_type(self):
        with pliron.Context():
            u = pliron.UnitType.get()
            assert "unit" in str(u).lower()

    def test_type_equality_and_hash(self):
        """Same type parameters → same uniqued type."""
        with pliron.Context():
            a = pliron.IntegerType.get(32, "signed")
            b = pliron.IntegerType.get(32, "signed")
            assert a == b
            assert hash(a) == hash(b)

    def test_type_inequality(self):
        with pliron.Context():
            a = pliron.IntegerType.get(32)
            b = pliron.IntegerType.get(64)
            assert a != b

    def test_type_name(self):
        with pliron.Context():
            ty = pliron.IntegerType.get(16)
...
class TestAttributes:
    def test_string_attr(self):
        with pliron.Context():
            a = pliron.StringAttr.new("hello")
            assert "hello" in str(a)

    def test_string_attr_name(self):
        with pliron.Context():
            a = pliron.StringAttr.new("test")
            name = a.attr_name()
            assert "builtin" in name
    ...
    def test_integer_attr(self):
        with pliron.Context():
            i64 = pliron.IntegerType.get(64, "signed")
            a = pliron.IntegerAttr.new(42, i64)
            s = str(a)
            assert "42" in s
    ...
    def test_attr_verify(self):
        with pliron.Context():
            i64 = pliron.IntegerType.get(64, "signed")
            a = pliron.IntegerAttr.new(99, i64)
            a.verify()
...
class TestOps:
    def test_module_op_name(self):
        with pliron.Context():
            m = pliron.ModuleOp.new("my_mod").operation()
            assert m.op_name() == "builtin.module"

    def test_module_has_one_region(self):
        with pliron.Context():
            m = pliron.ModuleOp.new("m").operation()
    ...
    def test_func_op_name(self):
        with pliron.Context():
            fty = pliron.FunctionType.get([], [])
            f = pliron.FuncOp.new("my_fn", fty).operation()
            assert f.op_name() == "builtin.func"

    def test_func_has_region_and_entry_block(self):
        with pliron.Context():
            i32 = pliron.IntegerType.get(32)
            fty = pliron.FunctionType.get([i32], [])
            f = pliron.FuncOp.new("add", fty).operation()
    ...
    def test_func_entry_block_arguments_match_type(self):
        ...
        with pliron.Context():
            ...
            i64 = pliron.IntegerType.get(64, "signed")
            fty = pliron.FunctionType.get([i32, i64], [])
            f = pliron.FuncOp.new("multi_arg", fty).operation()
    ...
    def test_func_entry_block_label(self):
        with pliron.Context():
            fty = pliron.FunctionType.get([], [])
            f = pliron.FuncOp.new("labeled", fty).operation()
            entry = f.get_region(0).entry_block()
    ...
    def test_no_results_no_operands(self):
        with pliron.Context():
            m = pliron.ModuleOp.new("x").operation()
            assert m.num_results() == 0
            assert m.num_operands() == 0

    def test_op_printing(self):
        with pliron.Context():
            m = pliron.ModuleOp.new("printable").operation()
            s = str(m)
    ...
    def test_op_repr(self):
        with pliron.Context():
            m = pliron.ModuleOp.new("r").operation()
            assert repr(m) == str(m)
    ...
    def test_op_equality_and_hash(self):
        with pliron.Context():
            m = pliron.ModuleOp.new("eq_test").operation()
            ops = m.get_region(0).entry_block().get_ops()
    ...
    def test_op_inequality(self):
        with pliron.Context():
            m1 = pliron.ModuleOp.new("a").operation()
            m2 = pliron.ModuleOp.new("b").operation()
            assert m1 != m2
...
class TestOperationPlacement:
    def test_insert_at_back(self):
        with pliron.Context():
            ...
            fty2 = pliron.FunctionType.get([], [])
            func2 = pliron.FuncOp.new("bar", fty2).operation()
    ...
    def test_insert_after(self):
        with pliron.Context():
            module, func, _ = _make_simple_module()
            fty2 = pliron.FunctionType.get([], [])
            func2 = pliron.FuncOp.new("after_this", fty2).operation()
            mod_block = module.get_region(0).entry_block()
    ...
    def test_insert_before(self):
        with pliron.Context():
            module, func, _ = _make_simple_module()
            fty2 = pliron.FunctionType.get([], [])
            func2 = pliron.FuncOp.new("before_this", fty2).operation()
    ...
    def test_next_prev_op(self):
        with pliron.Context():
            module, func, _ = _make_simple_module()
            fty2 = pliron.FunctionType.get([], [])
            func2 = pliron.FuncOp.new("second", fty2).operation()
            mod_block = module.get_region(0).entry_block()
            func2.insert_at_back(mod_block)
...
class TestBasicBlock:
    def test_block_arguments(self):
        with pliron.Context():
            i32 = pliron.IntegerType.get(32)
            fty = pliron.FunctionType.get([i32], [])
            f = pliron.FuncOp.new("with_args", fty).operation()
            entry = f.get_region(0).entry_block()
    ...
    def test_block_str(self):
        with pliron.Context():
            fty = pliron.FunctionType.get([], [])
            f = pliron.FuncOp.new("pr", fty).operation()
            entry = f.get_region(0).entry_block()
            s = str(entry)
    ...
    def test_block_ops_iterable(self):
        ...
        with pliron.Context():
            ...
            fty = pliron.FunctionType.get([], [])
            for name in ("a", "b"):
                pliron.FuncOp.new(name, fty).operation().insert_at_back(mod_block)

            snapshot = [op.op_name() for op in mod_block.get_ops()]
            ...
            it = mod_block.ops()
            assert next(it) == mod_block.get_ops()[0]

    def test_block_equality(self):
        with pliron.Context():
            fty = pliron.FunctionType.get([], [])
            f = pliron.FuncOp.new("eq", fty).operation()
            entry = f.get_region(0).entry_block()
...
class TestValue:
    def test_block_arg_value(self):
        with pliron.Context():
            i32 = pliron.IntegerType.get(32)
            fty = pliron.FunctionType.get([i32], [])
            f = pliron.FuncOp.new("val_test", fty).operation()
            entry = f.get_region(0).entry_block()
            arg = entry.get_argument(0)
    ...
    def test_value_str(self):
        with pliron.Context():
            i32 = pliron.IntegerType.get(32)
            fty = pliron.FunctionType.get([i32], [])
            f = pliron.FuncOp.new("vstr", fty).operation()
            entry = f.get_region(0).entry_block()
            arg = entry.get_argument(0)
            s = str(arg)
            assert isinstance(s, str)
    ...
    def test_value_equality(self):
        with pliron.Context():
            i32 = pliron.IntegerType.get(32)
            fty = pliron.FunctionType.get([i32], [])
            f = pliron.FuncOp.new("veq", fty).operation()
            entry = f.get_region(0).entry_block()
    ...
    def test_value_not_used(self):
        with pliron.Context():
            i32 = pliron.IntegerType.get(32)
            fty = pliron.FunctionType.get([i32], [])
            f = pliron.FuncOp.new("unused_arg", fty).operation()
            entry = f.get_region(0).entry_block()
            arg = entry.get_argument(0)
...
class TestOperationAttributes:
    def test_set_and_get_attribute(self):
        with pliron.Context():
            module, func, _ = _make_simple_module()
            attr = pliron.StringAttr.new("my_value")
    ...
    def test_get_nonexistent_attribute(self):
        with pliron.Context():
            m = pliron.ModuleOp.new("attr_test").operation()
    ...
    def test_integer_attr_on_op(self):
        with pliron.Context():
            module, func, _ = _make_simple_module()
            i32 = pliron.IntegerType.get(32)
            attr = pliron.IntegerAttr.new(100, i32)
...
class TestIRBuilder:
    def test_builder_at_block_end(self):
        with pliron.Context():
            module, func, _ = _make_simple_module()
            mod_block = module.get_region(0).entry_block()
            fty2 = pliron.FunctionType.get([], [])
            func2 = pliron.FuncOp.new("built", fty2).operation()
            builder = pliron.IRBuilder.at_block_end(mod_block)
            builder.append_op(func2)
            ops = mod_block.get_ops()
            assert len(ops) == 2
            assert ops[1] == func2

    def test_builder_at_block_start(self):
        with pliron.Context():
            module, func, _ = _make_simple_module()
            mod_block = module.get_region(0).entry_block()
            fty2 = pliron.FunctionType.get([], [])
            func2 = pliron.FuncOp.new("first", fty2).operation()
            builder = pliron.IRBuilder.at_block_start(mod_block)
            builder.append_op(func2)
            ops = mod_block.get_ops()
            assert ops[0] == func2

    def test_builder_after_op(self):
        with pliron.Context():
            module, func, _ = _make_simple_module()
            mod_block = module.get_region(0).entry_block()
            fty2 = pliron.FunctionType.get([], [])
            func2 = pliron.FuncOp.new("after", fty2).operation()
            builder = pliron.IRBuilder.after_op(func)
            builder.append_op(func2)
            ops = mod_block.get_ops()
            assert ops[1] == func2

    def test_builder_create_block(self):
        with pliron.Context():
            fty = pliron.FunctionType.get([], [])
            f = pliron.FuncOp.new("multi_block", fty).operation()
    ...
    def test_builder_set_insertion_point(self):
        with pliron.Context():
            module, func, _ = _make_simple_module()
            mod_block = module.get_region(0).entry_block()
            builder = pliron.IRBuilder.at_block_start(mod_block)
            # Change to end
            builder.set_at_block_end(mod_block)
            fty2 = pliron.FunctionType.get([], [])
            func2 = pliron.FuncOp.new("moved", fty2).operation()
            builder.append_op(func2)
            ops = mod_block.get_ops()
            assert ops[-1] == func2
...
class TestOperationErase:
    def test_erase_op(self):
        ...
        with pliron.Context() as ctx:
            ...
            fty2 = pliron.FunctionType.get([], [])
            func2 = pliron.FuncOp.new("survivor", fty2).operation()
            func2.insert_at_back(mod_block)
            assert len(mod_block.get_ops()) == 2
...
class TestPrinting:
    ...
    def test_value_repr_eq_str(self):
        with pliron.Context():
            i32 = pliron.IntegerType.get(32)
            fty = pliron.FunctionType.get([i32], [])
            f = pliron.FuncOp.new("repr_eq", fty).operation()
...
class TestVerification:
    ...
    def test_standalone_func_verifies_missing_terminator(self):
        with pliron.Context():
            fty = pliron.FunctionType.get([], [])
            f = pliron.FuncOp.new("standalone", fty).operation()
            with pytest.raises(pliron.PlironError, match="missing a terminator"):
                f.verify()
    ...
    def test_region_verifies_missing_terminator(self):
        with pliron.Context():
            fty = pliron.FunctionType.get([], [])
            f = pliron.FuncOp.new("rv", fty).operation()
            with pytest.raises(pliron.PlironError, match="missing a terminator"):
    ...
    def test_block_verifies_missing_terminator(self):
        with pliron.Context():
            fty = pliron.FunctionType.get([], [])
            f = pliron.FuncOp.new("bv", fty).operation()
            with pytest.raises(pliron.PlironError, match="missing a terminator"):
    ...
    def test_value_verifies(self):
        with pliron.Context():
            i32 = pliron.IntegerType.get(32)
            fty = pliron.FunctionType.get([i32], [])
            f = pliron.FuncOp.new("vv", fty).operation()
...
class TestIREmptiness:
    def test_context_not_empty_after_creating_ir(self):
        with pliron.Context() as ctx:
            pliron.ModuleOp.new("m").operation()
...
class TestModuleOnly:
    def test_module_only_verifies(self):
        ...
        with pliron.Context():
            m = pliron.ModuleOp.new("simple").operation()
            m.verify()

...
### `PyType` — the generic face (`pliron-python/src/types.rs`)
...
`verify_interfaces`, `instantiate`, raw `hash_type`/`get_self_handle` stay
internal):
...
A `#[pliron_type]` struct gets a `Py<Name>` wrapper — generated by
`pliron-python-derive` from its reflect export (or the stacked `#[py_type]`
attribute) — that **holds a `TypedHandle<Name>` and behaves like one**. Because
the handle is statically typed, `self.ptr.deref(ctx)` is a `Ref<Name>` — field
getters and mirrored `impl`-block methods call straight into the concrete Rust
type, with no runtime downcast.
...
- `from_type(ty: Type) -> Optional[Self]` (`@staticmethod`) — downcast a generic
  ...
  only when there is no active context. (Rust-side, the downcast check is
  followed by `TypedHandle::from_handle_unchecked` — one of the small
  Python-agnostic API additions to `pliron`.)
- `__str__`/`__repr__`/`__eq__`/`__hash__` — via the underlying `TypeHandle`.

`#[pliron_type_impl]` methods are exposed the same way via the impl mirror
(`02` §"impl-block methods"): instance methods deref `TypedHandle<T> → Ref<T>`
and call the native method; `&Context`/`&mut Context` is dropped and supplied
...
The blanket impl (in `pliron-python`) is lifted from two traits whose impls the
codegen emits per type (orphan-rule workaround — both `PyMap` and `TypedHandle`
are foreign to a dialect crate):
...
The generated type class emits `impl PyTypeWrapper for Py<Name>` (with
...
[`type_handle_from_any`](../src/types.rs) — it accepts a generic `Type`
...
These live alongside the type impls in [`pliron-python/src/py_map.rs`](../src/py_map.rs).

## Adding a new type (cookbook)

1. Define it with `#[pliron_type(name = "dialect.foo", …)]` and generate the
   wrapper — stack `#[py_type]` above the definition, or invoke
   `__pliron_reflect_ty_Foo!(pliron_python::derive::py_type_from_export)` from a
   bindings crate (see [05-extending.md](05-extending.md)). You get
...
3. Expose the constructor: a hand-written `#[staticmethod] get(...)` that returns
   `Py<Foo> { ptr: <TypedHandle from Foo::get> }`, or mirror the `impl` block
   with `#[py_type_impl]` / `py_type_impl_from_export!`.

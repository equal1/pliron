...
- **Rewrite bridge untested.** `apply_match_rewrite` + `Rewriter` are implemented
  but not covered by `tests/test_bindings.py`.
- **Rewriter lifetime is pointer-based.** `PyRewriter` holds a raw
  `*mut MatchRewriter` valid only during a callback; it is nulled on return so
  misuse raises rather than corrupts. Correctness depends on the GIL serializing
  callbacks.
